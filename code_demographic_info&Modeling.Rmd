...
# Step 1: generate gtsummary table
...
#   )
# 
# # Step 2: transform gtsummary table to flextable
# traumatic_events_flextable <- traumatic_events_table %>%
#   as_flex_table() %>%
#   flextable::autofit()
# 
# # Step 3: indent manually
# traumatic_events_flextable <- traumatic_events_flextable %>%
#   flextable::padding(
#     i = which(traumatic_events_table$table_body$indent == 1),  # find lines to indent
#     padding.left = 20,  # set left indent
#     part = "body"
#   )
# 
# # Step 4: Bold the "Traumatic Events on Duty" line
# traumatic_events_flextable <- traumatic_events_flextable %>%
#   flextable::bold(
#     i =  which(traumatic_events_table$table_body$variable == "Traumatic.events.on.duty"),  # confirm the bolded line 
#     part = "body"
#   )
```
...
# gtsummary table (should slightly change the column names)
...
```{r}
# Step 1: get the body of each FLEX table
...
## prepare dataset
Before running this chunk, run the chunks in air and wipe dataset code, including box plots and deleting non-significant elements.
...
## models for all  mental health variables
...
simple_ASI <- lm(sqrt(ASI) ~ sum_air, data = df)
summary(simple_ASI)
...
#
poisson_ASI <- glm(ASI ~ sum_wipe + FH_Yes, data = df, family = poisson)
summary(poisson_ASI)

#
poisson_ASI <- glm(ASI ~ sum_wipe + `Avg hrs on fire scenes per week`, data = df, family = poisson)
summary(poisson_ASI)

#
poisson_ASI <- glm(ASI ~ sum_wipe + `Years as a Firefighter`, data = df, family = poisson)
summary(poisson_ASI)
...
nb_ASI <- glm.nb(ASI ~ sum_air, data = df)
summary(nb_ASI)
...
# # log
# simple_ASI <- lm(log(ASI+1) ~ sum_air + sum_wipe, data = df)
# summary(simple_ASI)
# 
# # log
# simple_ASI_s <- lm(log(ASI_s+1) ~ sum_air + sum_wipe, data = df)
# summary(simple_ASI_s)
# 
# # log
# simple_ASI_s <- lm(log(ASI_s+1) ~ sum_air, data = df)
# summary(simple_ASI_s)
...
# summary(poisson_AvgHrs)
```
### check the assumption for poisson distribution
```{r}
poisson_PCL <- glm(PCL ~ sum_wipe, data = df, family = poisson)
summary(poisson_PCL)
PCL <- df$PCL
...
E.PCL <- predict(poisson_PCL,type="response")

par(mfrow=c(2,2))
plot(E.PCL, PCL - E.PCL)
plot(E.PCL, (PCL - E.PCL)/sqrt(E.PCL))
plot(PCL - E.PCL)
plot((PCL - E.PCL)/sqrt(E.PCL))
...
E.ASI <- predict(poisson_ASI,type="response")

par(mfrow=c(2,2))
plot(E.ASI, ASI - E.ASI)
plot(E.ASI, (ASI - E.ASI)/sqrt(E.ASI))
plot(ASI - E.ASI)
plot((ASI - E.ASI)/sqrt(E.ASI))
...
# Linearity 
plot(simple_ASI, which = 1)  # Residuals vs Fitted

# Independence
durbinWatsonTest(simple_ASI)

# constant variance
plot(simple_ASI, which = 3)  # Scale-Location

# Normality
plot(simple_ASI, which = 2)  # Q-Q plot
# Normality - Shapiro-Wilk test
residuals_simple_ASI <- residuals(simple_ASI)
shapiro.test(residuals_simple_ASI)
...
```{r}
simple_ASI_s <- lm(sqrt(ASI_s) ~ sum_air, data = df_1)
summary(simple_ASI_s)

# Linearity 
plot(simple_ASI_s, which = 1)  # Residuals vs Fitted

# Independence
durbinWatsonTest(simple_ASI_s)

# constant variance
plot(simple_ASI_s, which = 3)  # Scale-Location

# Normality
plot(simple_ASI_s, which = 2)  # Q-Q plot
# Normality - Shapiro-Wilk test
residuals_simple_ASI <- residuals(simple_ASI_s)
shapiro.test(residuals_simple_ASI)
...
```{r}
simple_ASI_s <- lm(sqrt(ASI_s) ~ sum_air + sum_wipe, data = df_1)
summary(simple_ASI_s)
